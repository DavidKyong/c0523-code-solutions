...
export default function Todos() {
  ...
  async function addTodo(newTodo) {
    try {
      ...
      setTodos((prev) => prev.concat(createTodo));
    } catch (error) {
      console.error(error);
    }
  }
  ...
  async function toggleCompleted(todoId) {
    const findTodo = todos.find((todos) => todos.todoId === todoId);
    try {
      const response = await fetch(`/api/todos/${todoId}`, {
        ...
        body: JSON.stringify({ isCompleted: !findTodo.isCompleted }),
      });
      if (!response.ok) {
        throw new Error(`Fetch error ${response.status}`);
      }
      const id = await response.json();
      const update = todos.map((old) => (old.todoId === id.todoId ? id : old));
      setTodos(update);
    } catch (error) {
      console.error(error);
    }
